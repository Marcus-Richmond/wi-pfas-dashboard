# Wisconsin PFAS Dashboard

Early-stage data cleaning and exploration for a Wisconsin PFAS environmental dashboard.

## Current Focus

Cleaning the Wisconsin DNR Surface Water and Fish Tissue PFAS sampling layer for later use in a web GIS dashboard.

## Data

Raw source data is not committed to this repository. To run the notebook, export the Surface Water and Fish Tissue layer from the Wisconsin DNR PFAS viewer and place the CSV at:

`data/raw/PFAS Sample Sites - Surface Water and Fish Tissue.csv`

## Notebooks

- `notebooks/Surface Water and Fish Tissue.ipynb`
